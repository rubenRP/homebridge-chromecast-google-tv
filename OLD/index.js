...
function ControlChromecastPlatform(log, config, api) {
  if (config) {
    ...
    if (this.config) this.ignoredDevices = this.config.ignoredDevices || [];
...
ControlChromecastPlatform.prototype.scanAccesories = function () {
  let addChromecast = function (device) {
    ...
    if (
      ...
      if (
        this.ignoredDevices &&
        this.ignoredDevices.indexOf(device.txtRecord.fn) !== -1
      ) {
        ...
        this.log(
          "Adding a new found Chomecast: %s [%s]",
          device.txtRecord.fn,
          device.txtRecord.id
        );
        ...
        if (typeof this.accessories[uuid].updateInfo === "function") {
          ...
          this.accessories[uuid] = new ChromecastAccessory(
            this.log,
            accessory,
            device
          );
...
ControlChromecastPlatform.prototype.addAccessory = function (device) {
  ...
  let accessory = new PlatformAccessory(
    device.name,
    UUIDGen.generate(device.txtRecord.id)
  );
  ...
  else accessory.category = this.api.hap.Categories.TELEVISION;
  ...
  this.accessories[accessory.UUID] = new ChromecastAccessory(
    this.log,
    accessory,
    device
  );
  ...
  this.api.publishExternalAccessories("homebridge-chromecast-google-tv", [
    accessory,
  ]);
};
...
ControlChromecastPlatform.prototype.configurationRequestHandler = function (
  context,
  request,
  callback
) {
  ...
  let sortAccessories = function () {
    context.sortedAccessories = Object.keys(this.accessories)
      .map(function (k) {
        return this[k] instanceof PlatformAccessory
          ? this[k]
          : this[k].accessory;
      }, this.accessories)
      .sort(function (a, b) {
  ...
  switch (context.onScreen) {
    ...
    case "ModifyAccessory":
      ...
      for (var index in context[`can${context.onScreen}`]) {
        ...
        var characteristic = new (Function.prototype.bind.apply(
          context[`can${context.onScreen}`][index],
          arguments
        ))();
    ...
    case "RemoveService":
      if (request.response.selections) {
        ...
        for (var i in request.response.selections.sort()) {
          let item =
            context[`can${context.onScreen}`][request.response.selections[i]];
          ...
          switch (context.onScreen) {
            case "AddCharacteristic":
              ...
              if (
                this.accessories[context.accessory.UUID] instanceof
                ChromecastAccessory
              ) {
                this.accessories[context.accessory.UUID].addEventHandler(
                  service,
                  item
                );
              }
            ...
            case "AddService":
              if (context.accessory.getService(item) === undefined) {
                context.accessory.addService(
                  item,
                  context.accessory.context.name
                );
              }
            ...
            case "RemoveService":
              if (context.accessory.getService(item) !== undefined) {
                context.accessory.removeService(
                  context.accessory.getService(item)
                );
              }
          }
        }
        ...
        respDict = {
          ...
          title: "Finished",
          detail: `Accessory ${
            /Service$/.test(context.onScreen) ? "service" : "characteristic"
          } ${
            /^Remove/.test(context.onScreen) ? "removal" : "addition"
          } was successful.`,
        };
    ...
    case "Configuration":
      respDict = {
        ...
        allowMultipleSelection: false,
        items:
          this.ignoredDevices && this.ignoredDevices.length > 0
...
function ChromecastAccessory(log, accessory, device) {
  ...
  this.log(
    `Controlling chromecast on ${this.chromecastIp}:${this.chromecastPort}`
  );
  ...
  if (service.getCharacteristic(Characteristic.Name).value === undefined) {
    service
      ...
      .setValue(this.accessory.context.name);
  }
...
ChromecastAccessory.prototype.clientConnect = function () {
  ...
  this.chromecastClient.connect(connectionDetails, () => {
    if (
      ...
      this.chromecastClient.getStatus((err, status) =>
        this.processClientStatus(status)
      );
    }
  });
};
...
ChromecastAccessory.prototype.clientError = function (error) {
  ...
  if (
    error_string.includes("EHOSTUNREACH") ||
    error_string.includes("ETIMEDOUT")
  )
...
ChromecastAccessory.prototype.processClientStatus = function (status) {
  ...
  // this.log("processClientStatus() - status", status);
      ...
      try {
        this.chromecastClient.join(
          ...
          (_, media) => {
            ...
            media.getStatus((err, mediaStatus) =>
              this.processMediaStatus(mediaStatus)
            );
  ...
  // Process power
  if (status.isStandBy) {
    this.log("processClientStatus() - Chromecast is off");
    this.setIsCasting(false);
  } else {
    this.log("processClientStatus() - Chromecast is on");
...
ChromecastAccessory.prototype.setVolume = function (volume, callback) {
  ...
  if (this.chromecastClient) {
    try {
      this.chromecastClient.setVolume({ level: volume / 100 }, () =>
        callback()
      );
    } catch (e) {
...
ChromecastAccessory.prototype.setDefaultProperties = function (
...
ChromecastAccessory.prototype.addEventHandler = function (
  service,
  characteristic
) {
  ...
  switch (characteristic) {
    ...
    case CustomCharacteristics.DeviceType:
      service;
      addCharacteristic(CustomCharacteristics.DeviceType).on(
        "get",
        (callback) => callback(null, this.deviceType)
      );
      break;
...
ChromecastAccessory.prototype.addEventHandlers = function () {
  ...
  this.accessoryInformationService
    ...
    .setCharacteristic(
      ...
      pkginfo.author.name || pkginfo.author
    )
...
module.exports = function (homebridge) {
  ...
  homebridge.registerPlatform(
    ...
    "ChromecastTelevision",
    ControlChromecastPlatform,
    true
  );
